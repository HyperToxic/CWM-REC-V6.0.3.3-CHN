...
void show_guohowhelp_menu()
{
   ...
   static char* headers[] = {   "\n",
    ...
    char* install_menu_items[] = {  "----知道了，请返回----",
                                                    NULL };
...
void show_choose_zip_menu(const char *mount_point)
{
    ...
    sprintf(confirm, "似滴 - 安装 %s", basename(file));
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    if (confirm_selection("确定恢复?", "对-恢复"))
...
void show_nandroid_delete_menu(const char* path)
{
    ...
    if (confirm_selection("确定删除?", "似滴-删除")) {
        // nandroid_restore(file, 1, 1, 1, 1, 1, 0);
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 1:
                ...
                if (confirm_selection( "确定清除?", "好的-清除 Dalvik Cache")) {
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log 已经被复制到 /sdcard/clockworkmod/recovery.log. 请sina提交给@guohow&bpok\n");
}
...
int verify_root_and_recovery() {
    ...
    if (0 == lstat("/system/etc/install-recovery.sh", &st)) {
        if (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)) {
            ui_show_text(1);
            ret = 1;
            if (confirm_selection("ROM重启时或许会恢复官方recovery. 修复?", "对-帮我组织恢复官方rec")) {
    ...
    if (0 == lstat("/system/bin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("ROOT权限可能丢失. 修复?", "好-帮我修复ROOT (/system/bin/su)")) {
    ...
    if (0 == lstat("/system/xbin/su", &st)) {
        ...
        if (S_ISREG(st.st_mode)) {
            if ((st.st_mode & (S_ISUID | S_ISGID)) != (S_ISUID | S_ISGID)) {
                ui_show_text(1);
                ret = 1;
                if (confirm_selection("ROOT权限可能丢失. 修复?", "好-帮我修复ROOT (/system/xbin/su)")) {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("Root权限丢失. Root您的设备?", "好-帮我ROOT (/system/xbin/su)")) {
